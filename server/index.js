...
const upload = multer({ 
  storage: storage,
  limits: {
    fileSize: 100 * 1024 * 1024, // 100MB 每個文件 (提高文件大小限制)
    files: 100, // 100個文件總數限制 (提高文件數量限制)
    fieldSize: 2 * 1024 * 1024, // 2MB 字段大小限制
    fields: 20, // 最多20個字段
  },
  ...
  upload.array('videos', 100)(req, res, async (err) => {
    if (err) {
      ...
      if (err.code === 'LIMIT_FILE_COUNT') {
        errorMessage = 'Too many files. Maximum 100 files allowed per upload.';
      } else if (err.code === 'LIMIT_FILE_SIZE') {
        errorMessage = 'File too large. Maximum 100MB per file.';
      } else if (err.code === 'LIMIT_UNEXPECTED_FILE') {
        errorMessage = 'Unexpected field name in form data.';
      } else if (err.code === 'LIMIT_PART_COUNT') {
        errorMessage = 'Too many parts in form data.';
        ...
        errorMessage = 'Only video files are allowed. Please select video files only.';
      }
